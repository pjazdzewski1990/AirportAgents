...
public class PassangerBehaviour extends AirportBaseBehaviour {
	...
	private void messageHandler(ACLMessage msg) {
		...
		case FLIGHT_TABLE_DATA:
			...
			//tutaj trzeba siê zastanowiæ nad wybieraniem lotów, narazie bierzemy pierwszy z brzegu i rezerwujemy
		//	System.out.println("Dostêpne loty:");
			//presentFlightData(msg.getContent().split(";"));
			
			
			//System.out.println("Rezerwacja lotu: " + msg.getContent().split(";")[0]);
			reserveFlight(getIndividualByName(msg.getContent().split(";")[0]), msg.getConversationId());
			
			
			//testowo zacznijmy kolejn¹ rezerwacje dla tego samego agenta podczas trwania innej rezerwacji
			if(test==1) {
			askForFlight();
			test=2;
			}
	...
	public void reserveFlight(OWLNamedIndividual reservation, String convId) {
		ACLMessage msg = new ACLMessage(ACLMessage.QUERY_IF);
		msg.addReceiver(new AID(AgentAddresses.getPassangerServiceAgentAddress(), AID.ISLOCALNAME));
		msg.setConversationId(HelperMethods.switchTag(StringMessages.RESERVATION, convId));
		printStatus(msg);
		msg.setLanguage(AgentAddresses.getLang());
		msg.setOntology(Constants.ontoURL);
	...
	private void printStatus(ACLMessage msg) { 
		...
		if(resData[0]=="null") {
			...
			if(HelperMethods.getConvId(resData[0]).equals(HelperMethods.getConvId(msg.getConversationId()))) {
			resData[0] = msg.getConversationId();
			} else {
